...
export default class GameReader {
  ...
  async checkProcessOpen(): Promise<void> {
    const processesOpen = getProcesses().filter(
      (p: { szExeFile: string }) => p.szExeFile === "Among Us.exe"
    );
    let error = "";
    const reset =
      this.amongUs &&
      processesOpen.filter(
        (o: { th32ProcessID: number }) => o.th32ProcessID === this.pid
      ).length === 0;
  ...
  getInstalledMods(filePath: string): AmongusMod {
    ...
    if (pathLower.includes("?\\volume") || this.is_linux) {
      ...
      if (
        !fs.existsSync(path.join(dir, "winhttp.dll")) ||
        !fs.existsSync(path.join(dir, "BepInEx\\plugins"))
      ) {
        return modList[0];
      }
      for (const file of fs.readdirSync(path.join(dir, "BepInEx\\plugins"))) {
        console.log(`MOD! ${file}`);
        const mod = modList.find(
          (o) => o.dllStartsWith && file.includes(o.dllStartsWith)
        );
        if (mod) return mod;
      }
      return modList[0];
  ...
  async loop(): Promise<string | null> {
    ...
    if (
      ...
      const gameState = this.readMemory<number>(
        "int",
        innerNetClient,
        this.offsets.innerNetClient.gameState
      );
      ...
      const lobbyCodeInt =
        state === GameState.MENU
          ...
          : this.readMemory<number>(
              "int32",
              innerNetClient,
              this.offsets.innerNetClient.gameId
            );
      ...
      const hostId = this.readMemory<number>(
        "uint32",
        innerNetClient,
        this.offsets.innerNetClient.hostId
      );
      const clientId = this.readMemory<number>(
        "uint32",
        innerNetClient,
        this.offsets.innerNetClient.clientId
      );
      ...
      if ((this.gameCode || this.isLocalGame) && playerCount) {
        ...
        if (localPlayer) {
          this.fixPingMessage();
          lightRadius = this.readMemory<number>(
            "float",
            localPlayer.objectPtr,
            this.offsets.lightRadius,
            -1
          );
        }
        ...
        maxPlayers = this.readMemory<number>(
          "byte",
          gameOptionsPtr,
        ...
        map = this.readMemory<number>(
          "byte",
          gameOptionsPtr,
          this.offsets.gameOptions_MapId
        );
        ...
        if (state === GameState.TASKS) {
          ...
          const systemsPtr = this.readMemory<number>(
            "ptr",
            shipPtr,
            this.offsets.shipStatus_systems
          );
          ...
          if (systemsPtr !== 0 && state === GameState.TASKS) {
            this.readDictionary(systemsPtr, 47, (k, v) => {
              ...
              if (key === 14) {
                  ...
                  case MapType.SUBMERGED: {
                    comsSabotaged =
                      this.readMemory<number>(
                        "uint32",
                        value,
                        this.offsets!.HudOverrideSystemType_isActive
                      ) === 1;
                    break;
                  }
                  case MapType.MIRA_HQ: {
                    comsSabotaged =
                      this.readMemory<number>(
                        "uint32",
                        value,
                        this.offsets!.hqHudSystemType_CompletedConsoles
                      ) < 2;
                  }
                }
                ...
                const value = this.readMemory<number>("ptr", v);
                const lowerDoorOpen = this.readMemory<number>(
                  "int",
                  value,
                  this.offsets!.deconDoorLowerOpen
                );
                const upperDoorOpen = this.readMemory<number>(
                  "int",
                  value,
                  this.offsets!.deconDoorUpperOpen
                );
                if (!lowerDoorOpen) {
                  closedDoors.push(0);
                }
          ...
          const minigamePtr = this.readMemory<number>(
            "ptr",
            this.gameAssembly.modBaseAddr,
            this.offsets!.miniGame
          );
          const minigameCachePtr = this.readMemory<number>(
            "ptr",
            minigamePtr,
            this.offsets!.objectCachePtr
          );
            ...
            if (map === MapType.POLUS || map === MapType.AIRSHIP) {
              ...
              if (roomCount === 4) {
                const dist = Math.sqrt(
                  Math.pow(localPlayer.x - -12.9364, 2) +
                    Math.pow(localPlayer.y - -2.7928, 2)
                );
                if (dist < 0.6) {
          ...
          if (map !== MapType.MIRA_HQ) {
            const allDoors = this.readMemory<number>(
              "ptr",
              shipPtr,
              this.offsets.shipstatus_allDoors
            );
            ...
            for (let doorNr = 0; doorNr < doorCount; doorNr++) {
              const door = this.readMemory<number>(
                "ptr",
                allDoors +
                  this.offsets.playerAddrPtr +
                  doorNr * (this.is_64bit ? 0x8 : 0x4)
              );
              const doorOpen =
                this.readMemory<number>(
                  "int",
                  door + this.offsets.door_isOpen
                ) === 1;
  ...
  async initializeoffsets(): Promise<void> {
    ...
    const broadcastVersion = this.readMemory<number>(
      "int",
      this.gameAssembly!.modBaseAddr,
    ...
    if (offsetLookups.versions[broadcastVersion]) {
      this.offsets = await fetchOffsetsJson(
        this.is_64bit,
    ...
    console.log(
      ...
      this.offsets.serverManager_currentServer[0].toString(16)
    );
  ...
  initializeWrites(): void {
    ...
    const shellCodeAddr = virtualAllocEx(
      this.amongUs.handle,
      null,
      0x60,
    ...
    const connectFunc =
      this.gameAssembly.modBaseAddr + this.offsets.connectFunc;
    const relativeConnectJMP = connectFunc - (shellCodeAddr + 0x18) - 0x4;

    const fixedUpdateFunc =
      this.gameAssembly!.modBaseAddr + this.offsets.fixedUpdateFunc;
    const relativefixedJMP =
      fixedUpdateFunc + 0x5 - (shellCodeAddr + 0x24) - 0x4;
    ...
    const modManagerLateUpdate =
      this.gameAssembly!.modBaseAddr + this.offsets.modLateUpdateFunc;
    const shellCodeAddr_1 = shellCodeAddr + 0x300;
    const relativeShellJMP_1 =
      shellCodeAddr_1 - (modManagerLateUpdate + 0x1) - 0x4;
    const relativefixedJMP_1 =
      modManagerLateUpdate + 0x5 - (shellCodeAddr_1 + 0x1c) - 0x4;
    const showModStampFunc =
      this.gameAssembly!.modBaseAddr + this.offsets.showModStampFunc;
    const relativeShowModStamp =
      showModStampFunc + 0x6 - (shellCodeAddr_1 + 0x12) - 0x4;
    ...
    writeBuffer(
      this.amongUs!.handle,
      fixedUpdateFunc,
      Buffer.from(shellcodeJMP)
    );

    writeBuffer(
      this.amongUs!.handle,
      shellCodeAddr_1,
      Buffer.from(shellcode_modIcon)
    );
    writeBuffer(
      this.amongUs!.handle,
      modManagerLateUpdate,
      Buffer.from(shellcodeJMP_1)
    );
  ...
  fixPingMessage() {
    ...
    for (let index = 0; index < 3; index++) {
      ...
      const stringPtr = this.readMemory<number>(
        "int",
        this.gameAssembly.modBaseAddr,
        stringOffset
      );
      ...
      if (
        pingstring.includes("Ping") ||
        pingstring.includes("<color=#BA68C8")
      ) {
        writeMemory(
          this.amongUs!.handle,
  ...
  loadColors(): void {
    ...
    const palletePtr = this.readMemory<number>(
      "ptr",
      this.gameAssembly!.modBaseAddr,
      this.offsets!.palette
    );
    const PlayerColorsPtr = this.readMemory<number>(
      "ptr",
      palletePtr,
      this.offsets!.palette_playercolor
    );
    const ShadowColorsPtr = this.readMemory<number>(
      "ptr",
      palletePtr,
      this.offsets!.palette_shadowColor
    );

    const colorLength = this.readMemory<number>(
      "int",
      ShadowColorsPtr,
      this.offsets!.playerCount
    );
    ...
    if (
      ...
      colorLength <= 0 ||
      colorLength > 300 ||
      ((this.loadedMod.id == "THE_OTHER_ROLES" ||
        this.loadedMod.id == "THE_OTHER_ROLES_GM") &&
        colorLength <= 18)
    ) {
      return;
    }
    ...
    for (let i = 0; i < colorLength; i++) {
      const playerColor = this.readMemory<number>("uint32", PlayerColorsPtr, [
        this.offsets!.playerAddrPtr + i * 0x4,
      ]);
      const shadowColor = this.readMemory<number>("uint32", ShadowColorsPtr, [
        this.offsets!.playerAddrPtr + i * 0x4,
      ]);
      ...
      //4278190080
      playercolors[i] = [
        numberToColorHex(playerColor),
        numberToColorHex(shadowColor),
      ];
    }
    ...
    try {
      this.sendIPC(
        IpcOverlayMessages.NOTIFY_PLAYERCOLORS_CHANGED,
        playercolors
      );
  ...
  readDictionary(
    ...
    const entries = this.readMemory<number>(
      "ptr",
      address + (this.is_64bit ? 0x18 : 0xc)
    );
    let len = this.readMemory<number>(
      "uint32",
      address + (this.is_64bit ? 0x20 : 0x10)
    );
    ...
    for (let i = 0; i < len; i++) {
      const offset =
        entries +
        ((this.is_64bit ? 0x20 : 0x10) + i * (this.is_64bit ? 0x18 : 0x10));
  ...
  parsePlayer(
    ...
    if (this.is_64bit) {
      ...
      data.outfitsPtr = this.readMemory("pointer", ptr, [
        this.PlayerStruct.getOffsetByName("outfitsPtr"),
      ]);
      data.taskPtr = this.readMemory("pointer", ptr, [
        this.PlayerStruct.getOffsetByName("taskPtr"),
      ]);
      data.rolePtr = this.readMemory("pointer", ptr, [
        this.PlayerStruct.getOffsetByName("rolePtr"),
      ]);
    ...
    let x = this.readMemory<number>(
      "float",
      data.objectPtr,
      positionOffsets[0]
    );
    let y = this.readMemory<number>(
      "float",
      data.objectPtr,
      positionOffsets[1]
    );
    let currentOutfit = this.readMemory<number>(
      "uint32",
      data.objectPtr,
      this.offsets.player.currentOutfit
    );
    const isDummy = this.readMemory<boolean>(
      "boolean",
      data.objectPtr,
      this.offsets.player.isDummy
    );
    let name = "error";
    let shiftedColor = -1;
    if (data.hasOwnProperty("name")) {
      ...
      this.readDictionary(data.outfitsPtr, 6, (k, v, i) => {
        ...
        if (key === 0 && i == 0) {
          const namePtr = this.readMemory<number>(
            "pointer",
            val,
            this.offsets!.player.outfit.playerName
          ); // 0x40
          data.color = this.readMemory<number>(
            "uint32",
            val,
            this.offsets!.player.outfit.colorId
          ); // 0x14
          name = this.readString(namePtr).split(/<.*?>/).join("");
          data.hat = this.readString(
            this.readMemory<number>(
              "ptr",
              val,
              this.offsets!.player.outfit.hatId
            )
          );
          data.skin = this.readString(
            this.readMemory<number>(
              "ptr",
              val,
              this.offsets!.player.outfit.skinId
            )
          );
          data.visor = this.readString(
            this.readMemory<number>(
              "ptr",
              val,
              this.offsets!.player.outfit.visorId
            )
          );
          if (currentOutfit == 0 || currentOutfit > 10) return;
        } else if (key === currentOutfit) {
          shiftedColor = this.readMemory<number>(
            "uint32",
            val,
            this.offsets!.player.outfit.colorId
          ); // 0x14
      ...
      const roleTeam = this.readMemory<number>(
        "uint32",
        data.rolePtr,
        this.offsets!.player.roleTeam
      );
      data.impostor = roleTeam;

      if (
        this.offsets!.player.nameText &&
        shiftedColor == -1 &&
        (this.loadedMod.id == "THE_OTHER_ROLES" ||
          this.loadedMod.id == "THE_OTHER_ROLES_GM")
      ) {
        let nameText = this.readMemory<number>(
          "ptr",
          data.objectPtr,
          this.offsets!.player.nameText
        );
        let nameText_name = this.readString(nameText);
    ...
    const colorId =
      data.color === this.rainbowColor ? RainbowColorId : data.color;
    return {
      ptr,
      id: data.id,
      clientId: clientId,
