...
async function colorImages(
  ...
  ); //`${app.getAppPath()}/../test/${imagename}.png`
...
async function colorImage(
  ...
  for (let i = 0, l = img.bitmap.data.length; i < l; i += 4) {
    ...
    if (
      h[1] > 0.4 &&
      (isBetween(h[0], 240, 30) ||
        isBetween(h[0], 0, 100) ||
        isBetween(h[0], 120, 40))
    ) {
      //  )
...
export async function GenerateAvatars(colors: string[][]): Promise<void> {
  console.log(
    "Generating avatars..",
    `${app.getPath("userData")}/static/generated/`
  );
  try {
...
export async function GenerateHat(
  imagePath: URL,
  colors: string[][],
  colorId: number,
  ...
  try {
    ...
    if (
      !fs.existsSync(temp) ||
      Date.now() - (await fs.promises.stat(temp)).mtimeMs > 300000
    ) {
